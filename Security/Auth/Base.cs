...
using HarmonyLib;
using parrot.Background.Mods;
using parrot.Components;
using UnityEngine;
using UnityEngine.XR;

namespace parrot.Background.Security.Auth
{
	...
	internal class Base
	{
		...
		public static void Init()
		{
			try
			{
				if (Harmony.HasAnyPatches("com.parrot.lol"))
				{
				...
				{
					Base.GetAuth.init();
					if (File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "parrot", "parrotkey.txt")))
					{
						string text = File.ReadAllText(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "parrot", "parrotkey.txt"));
						Base.key = text;
						Base.GetAuth.license2(text);
						if (Base.GetAuth.response.success)
						{
							if (!GameObject.Find("parrot"))
							{
								if (!new WebClient().DownloadString("https://bbc123f.github.io/killswitch.txt").Contains("="))
								{
									Base.ms = new GameObject("parrot");
									...
									new Harmony("com.parrot.lol").PatchAll();
									if (File.Exists("parrot_error.log"))
									{
										File.Delete("parrot_error.log");
									}
		...
		public static auth GetAuth = new auth("parrot", "RovpqveRf3", "28dd3f3d424e86309e9d467c19b5936e61cc0abbd55e3360a04334e6044b9144", "1.0");
